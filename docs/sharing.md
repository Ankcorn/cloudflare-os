...
- **Verification has no commit gate.** The redemption topology check and confirm, and the
  post-verification role re-check, run after `ensureObserver` already persisted the observer
  record, so a denial leaves the record behind -- and cannot stop the persist from resurrecting a
  record a removal's teardown just deleted.
- **No fail-closed revocation window.** Observations arriving between a removal's edge sever and
  the revocation restart are admitted rather than failed closed: every per-observation check
  reads the removed user as gone while their sessions stay live.
