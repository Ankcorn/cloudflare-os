...
<html lang="en">
  ...
  <body style="font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; background: #f5f5f5;">
    <div style="max-width: 520px; padding: 2rem; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); text-align: center;">
      ...
      <p style="color: #555; line-height: 1.6;">This authorization link is invalid or has expired. Please return to the Gadgets Workshop and try again.</p>
...
export class GatekeeperVendor extends WorkerEntrypoint<Env> implements GatekeeperVendorIface {
  async describe(): Promise<VendorDescription> {
    return {
      ...
      description:
          "Connect your Notion workspace to let Gadgets search, read, and edit the pages and " +
...
export class NotionWorkspaceGatekeeperImpl
  ...
  async startSession(approvalQueue: RpcStub<ApprovalQueue>): Promise<NotionWorkspaceSession> {
    ...
    const authorizedBy: NotionUser | null = info.ownerId
      ...
      : null;
    return new NotionWorkspaceSessionImpl(this.#store(), approvalQueue.dup(), authorizedBy);
  }
...
class NotionPageSessionImpl extends RpcTarget implements NotionPageSession {
  ...
  constructor(store: NotionStore, approvalQueue: RpcStub<ApprovalQueue>, pageId: string) {
    super();
    this.#store = store;
  ...
  async createSubPage(options: NotionCreatePageOptions): Promise<NotionPageSession> {
    ...
    return new NotionPageSessionImpl(this.#store, this.#approvalQueue.dup(), provisionalId);
  }
...
class NotionDatabaseSessionImpl extends RpcTarget implements NotionDatabaseSession {
  ...
  constructor(store: NotionStore, approvalQueue: RpcStub<ApprovalQueue>, databaseId: string) {
    super();
    this.#store = store;
    this.#approvalQueue = approvalQueue;
  ...
  async getPage(idOrUrl: string): Promise<NotionPageSession> {
    ...
    if (NotionStore.isProvisional(idOrUrl)) {
      ...
      return new NotionPageSessionImpl(this.#store, this.#approvalQueue.dup(), idOrUrl);
    }
    ...
    return new NotionPageSessionImpl(this.#store, this.#approvalQueue.dup(), id);
  }
...
class NotionWorkspaceSessionImpl extends RpcTarget implements NotionWorkspaceSession {
  ...
  constructor(store: NotionStore, approvalQueue: RpcStub<ApprovalQueue>, authorizedBy: NotionUser | null) {
    super();
    this.#store = store;
    this.#approvalQueue = approvalQueue;
  ...
  async getDatabase(idOrUrl: string): Promise<NotionDatabaseSession> {
    ...
    return new NotionDatabaseSessionImpl(this.#store, this.#approvalQueue.dup(), id);
  }
