...
export class GatekeeperVendor extends WorkerEntrypoint<Env> implements GatekeeperVendorIface {
  async describe(): Promise<VendorDescription> {
    return {
      ...
      description:
          "Connect your Slack account to give Gadgets read-only access to the workspaces, " +
  ...
  async addObserver(id: string, user: Fetcher<GatekeeperUserVerifier>): Promise<void> {
    ...
    while (true) {
      ...
      if (access.some(hasAccess => !hasAccess)) {
        throw new Error(
            "This collaborator does not have access to a Slack conversation whose data the Gadget " +
  ...
  async addObserver(_id: string, user: Fetcher<GatekeeperUserVerifier>): Promise<void> {
    ...
    if (!(await verifier.hasConversationAccess(this.ctx.props.conversationId))) {
      throw new Error(
          ...
          "thread, so they cannot be allowed to observe data the Gadget read from it.");
    }
  }
