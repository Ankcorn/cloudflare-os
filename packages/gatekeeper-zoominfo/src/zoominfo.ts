...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (relPath === "/oauth") {
      ...
      if (error) {
        return new Response(
          "ZoomInfo authorization failed or was denied. Please restart the connection flow from Gadgets.",
...
export class GatekeeperVendor extends WorkerEntrypoint<Env> implements GatekeeperVendorIface {
  async describe(): Promise<VendorDescription> {
    return {
      ...
      description:
        "Connect your ZoomInfo account so Gadgets can resolve filter values, search companies, " +
