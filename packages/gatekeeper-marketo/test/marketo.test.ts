...
describe("optional deployment defaults", () => {
  ...
  it("keeps replacement credentials and retries a lost reconnect callback response after restart", async () => {
    ...
    await runInDurableObject(account, async (instance, state) => {
      ...
      vi.spyOn(state.storage.kv, "get").mockImplementation((key: string) => key === "callback"
        ? { credentialsRestored: async () => {
            attempts++;
            throw new Error("Workshop response lost");
          } }
        : get(key));
      await expect(instance.completeConnection(nonce, {
        ...initial,
        clientSecret: "replacement-secret",
      })).resolves.toMatchObject({ kind: "error" });
      expect(instance.getCredentials()).toEqual({ ...initial, clientSecret: "replacement-secret" });
      expect(state.storage.kv.get("reconnecting")).toMatchObject({
        phase: "awaiting-notification",
      });
    });
    await expect(runInDurableObject(account, (_instance, state) => {
      state.abort("restart after lost reconnect callback response");
    })).rejects.toThrow(/restart after lost/);

    account = namespace.get(account.id);
    await runInDurableObject(account, async (instance, state) => {
      let get = state.storage.kv.get.bind(state.storage.kv);
      vi.spyOn(state.storage.kv, "get").mockImplementation((key: string) => key === "callback"
        ? { credentialsRestored: async () => { attempts++; } }
        : get(key));
      await instance.alarm();
      expect(state.storage.kv.get("reconnecting")).toBeUndefined();
    });
    expect(attempts).toBe(2);
    expect(await account.getCredentials()).toEqual({ ...initial, clientSecret: "replacement-secret" });
  });
  ...
  it("does not clean up a newer reconnect after an older callback succeeds", async () => {
    ...
    await runInDurableObject(account, async (instance, state) => {
      ...
      await started;
      expect(state.storage.kv.get<{ generation: number; phase: string }>("reconnecting"))
        .toMatchObject({
          generation: instance.getCredentialState().generation,
          phase: "awaiting-notification",
        });
  ...
  it("recovers a credential swap after restart before the Workshop callback", async () => {
    let namespace = (env as unknown as { UserAccount: DurableObjectNamespace<UserAccount> }).UserAccount;
    let account = namespace.get(namespace.newUniqueId());
    let replacement = { endpoint: ORIGIN, clientId: "saved-id", clientSecret: "replacement-secret" };

    await runInDurableObject(account, (_instance, state) => {
      state.storage.kv.put("credentials", replacement);
      state.storage.kv.put("credentialGeneration", 2);
      state.storage.kv.put("reconnecting", {
        value: "nonce", expiresAt: Date.now() - 1, generation: 2,
        phase: "awaiting-notification",
      });
    });
    await expect(runInDurableObject(account, (_instance, state) => {
      state.abort("simulate restart before credentialsRestored");
    })).rejects.toThrow(/simulate restart/);
    ...
    await runInDurableObject(account, async (instance, state) => {
      let get = state.storage.kv.get.bind(state.storage.kv);
      vi.spyOn(state.storage.kv, "get").mockImplementation((key: string) =>
        key === "callback" ? { credentialsRestored: async () => { notifications++; } } : get(key));
      await instance.alarm();
    });

    expect(notifications).toBe(1);
    expect(await account.getCredentialState()).toEqual({ credentials: replacement, generation: 2 });
    await runInDurableObject(account, (_instance, state) => {
      expect(state.storage.kv.get("reconnecting")).toBeUndefined();
      expect(state.storage.kv.get("nonce")).toBeUndefined();
    });
...
describe("Design Studio scoped binding", () => {
  ...
  it("retains and blocks an ambiguous staged row when queue submission fails", async () => {
    let accountId = await accountWithCredentials({
      endpoint: ORIGIN,
      clientId: "client",
      clientSecret: crypto.randomUUID(),
    ...
    await runInDurableObject(gatekeeper, async (instance, state) => {
      let queue = {
        ...
        async submitAction() {
          throw new Error("approval queue response lost");
        },
        [Symbol.dispose]() {},
      } as unknown as RpcStub<ApprovalQueue>;
      let session = await instance.startSession(queue);
      let studio = session as MarketoDesignStudioImpl;
      await expect(studio.getEmail("20").approve()).rejects.toThrow("approval queue response lost");
      expect(state.storage.kv.get("pending:1")).toBeUndefined();
      expect(state.storage.kv.get("staged:1")).toMatchObject({ state: "submission-uncertain" });
      expect(state.storage.kv.get("staged:index")).toEqual([1]);
      expect(state.storage.kv.get("applying:1")).toBeUndefined();

      let ordinarySession = await instance.startSession(queue);
      let ordinaryStudio = ordinarySession as MarketoDesignStudioImpl;
      expect(state.storage.kv.get("pending:1")).toBeUndefined();
      expect(state.storage.kv.get("staged:1")).toMatchObject({ state: "submission-uncertain" });
      expect(await ordinaryStudio.getEmail("20").describe()).toMatchObject({ status: "draft" });
      ordinarySession[Symbol.dispose]();

      expect(writes).toBe(0);
      pauseWrite = true;
      let applying = instance.applyAction(1);
      await started;
      expect(state.storage.kv.get("pending:1")).toMatchObject({ state: "dispatching" });
  ...
  it("stages an action without exposing its simulation, then promotes it on success", async () => {
    ...
    await runInDurableObject(gatekeeper, async (instance, state) => {
      ...
      expect(state.storage.kv.get("staged:1")).toMatchObject({ state: "submitting" });
  ...
  it("retains a promoted action while a callback prepares after queue submission fails", async () => {
    let accountId = await accountWithCredentials({
      endpoint: ORIGIN, clientId: "client", clientSecret: crypto.randomUUID(),
    });
    let gatekeeper = await gatekeeperForAccount(accountId.toString(), "design-studio");
    ...
    await runInDurableObject(gatekeeper, async (instance, state) => {
      ...
      expect(state.storage.kv.get("pending:1")).toMatchObject({
        state: "preflight",
      });
      expect(state.storage.kv.get("staged:1")).toBeUndefined();
      releasePreparation();
  ...
  it("blocks a promoted action after a definite apply failure", async () => {
    let accountId = await accountWithCredentials({
      endpoint: ORIGIN, clientId: "client", clientSecret: crypto.randomUUID(),
    });
    let gatekeeper = await gatekeeperForAccount(accountId.toString(), "design-studio");
    ...
    await runInDurableObject(gatekeeper, async (instance, state) => {
      ...
      expect(state.storage.kv.get("staged:1")).toBeUndefined();
      expect(state.storage.kv.get("applying:1")).toBeUndefined();
      await expect(instance.applyAction(1)).rejects.toThrow(/Approval registration failed/);
  ...
  it("repairs a partial registered promotion after restart and leaves pre-submit work invisible", async () => {
    let accountId = await accountWithCredentials({
      endpoint: ORIGIN, clientId: "client", clientSecret: crypto.randomUUID(),
    });
    let gatekeeper = await gatekeeperForAccount(accountId.toString(), "design-studio");
    ...
    await runInDurableObject(gatekeeper, (_instance, state) => {
      let parentAction: ProgramAction = {
        id: 1, type: "programCreate", provisionalId: "~1", parentId: "10",
        input: { name: "Recovered program", type: "Default", channel: "Email" },
      };
      state.storage.kv.put("staged:1", {
        action: parentAction,
        ownerGeneration: 0,
        state: "registered",
      });
      // Simulate a crash after pending:<id> was written but before pending:index and staged cleanup.
      state.storage.kv.put("pending:1", {
        action: parentAction,
        ownerGeneration: 0,
        state: "registered",
      });
      state.storage.kv.put("staged:2", {
        action: {
          id: 2, type: "campaignCreate", provisionalId: "~2",
          parent: { id: "~1", type: "Program" }, name: "Dependent campaign",
        },
        ownerGeneration: 0,
        state: "registered",
      });
      state.storage.kv.put("staged:3", {
        action: {
          id: 3, type: "designMetadata", asset: "email", targetId: "20",
          patch: { description: "Never submitted" },
        },
        ownerGeneration: 0,
        state: "prepared",
      });
      state.storage.kv.put("staged:index", [1, 2, 3]);
    });
    await expect(runInDurableObject(gatekeeper, (_instance, state) => {
      state.abort("simulate restart after submitAction");
    })).rejects.toThrow(/simulate restart/);

    let namespace = (env as unknown as {
      MarketoGatekeeperImpl: DurableObjectNamespace<MarketoGatekeeperImpl>;
    }).MarketoGatekeeperImpl;
    gatekeeper = namespace.get(gatekeeper.id);
    await runInDurableObject(gatekeeper, instance => {
      let ctx = (instance as unknown as { ctx: { props: {
        userObjectId: string;
        bindingId: string;
        kind: "instance";
      } } }).ctx;
      ctx.props = {
        userObjectId: accountId.toString(), bindingId: "restarted", kind: "instance",
      };
    });
    await runInDurableObject(gatekeeper, async (instance, state) => {
      let queue = new RpcStub(new TestApprovalQueue()) as unknown as RpcStub<ApprovalQueue>;
      let session = await instance.startSession(queue) as MarketoSessionImpl;
      let program = session.getProgram("~1");

      expect(await program.getTokens()).toEqual([]);
      expect(state.storage.kv.get<number[]>("pending:index")).toEqual([1, 2]);
      expect(state.storage.kv.get("staged:1")).toBeUndefined();
      expect(state.storage.kv.get("staged:2")).toBeUndefined();
      expect(state.storage.kv.get("pending:3")).toBeUndefined();
      expect(state.storage.kv.get("staged:3")).toMatchObject({ state: "prepared" });
      await expect(instance.applyAction(3)).rejects.toThrow(/never submitted/);
      await expect(instance.rejectAction(1)).resolves.toEqual({ restart: true });
      expect(state.storage.kv.get("pending:2")).toMatchObject({ state: "blocked", blockedBy: 1 });
  ...
  it("excludes abandoned stages from the limit and rejects callbacks after cleanup", async () => {
    let accountId = await accountWithCredentials({
      endpoint: ORIGIN, clientId: "client", clientSecret: crypto.randomUUID(),
    });
    let gatekeeper = await gatekeeperForAccount(accountId.toString(), "design-studio");
    ...
    await runInDurableObject(gatekeeper, async (instance, state) => {
      let abandoned = Array.from({ length: 205 }, (_, offset) => offset + 1);
      for (let id of abandoned) {
        state.storage.kv.put(`staged:${id}`, {
          action: {
            id, type: "designMetadata", asset: "email", targetId: String(id),
            patch: { description: "Abandoned" },
          },
          ownerGeneration: 0,
          state: id % 2 === 0 ? "prepared" : "submission-uncertain",
          ...(id === abandoned.length ? { expiresAt: Date.now() - 1 } : {}),
        });
      }
      state.storage.kv.put("staged:index", abandoned);
      state.storage.kv.put("counter:nextActionId", abandoned.length);

      let queue = new RpcStub(new TestApprovalQueue()) as unknown as RpcStub<ApprovalQueue>;
      let studio = await instance.startSession(queue) as MarketoDesignStudioImpl;
      expect(state.storage.kv.get<number[]>("staged:index")).toHaveLength(204);

      await studio.getEmail("20").updateMetadata({ description: "Still accepted" });
      expect(state.storage.kv.get("pending:206")).toBeDefined();
      await expect(instance.applyAction(205)).rejects.toThrow(/No queued Marketo action/);
      expect(writes).toBe(0);
