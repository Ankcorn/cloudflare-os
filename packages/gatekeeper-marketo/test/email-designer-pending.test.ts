...
describe("Email Designer pending-state simulation", () => {
  ...
  it.each([
    ["without identity filters", {}, 2],
    ["with disabled identity filters", { isCreatedByMe: false, isModifiedByMe: false }, 2],
    ["with isModifiedByMe", { isModifiedByMe: true }, undefined],
    ["with disabled isCreatedByMe and enabled isModifiedByMe", { isCreatedByMe: false, isModifiedByMe: true }, undefined],
  ] as const)("filters unresolved pending templates locally %s", async (_label, identityFilters, totalItems) => {
    let providerFilters = 0;
    let { ctx } = context({
      filterDesignerAssets: async () => {
        providerFilters++;
        throw new Error("An unresolved template filter reached Marketo.");
      },
      getDesignerAsset: async (_kind, assetId) => ({
        id: assetId, name: "Retargeted", templateId: "template-old",
        appData: { workspaceId: "1" },
      }),
    }, [
      {
        id: 1, type: "designerCreate", asset: "designerTemplate", provisionalId: "~template",
        body: { name: "Pending template" },
      },
      {
        id: 2, type: "designerCreate", asset: "designerEmail", provisionalId: "~matching",
        body: { name: "Created", templateId: "~template", appData: { workspaceId: "1" } },
      },
      {
        id: 3, type: "designerCreate", asset: "designerEmail", provisionalId: "~other",
        body: { name: "Other", templateId: "template-other", appData: { workspaceId: "1" } },
      },
      {
        id: 4, type: "designerUpdate", asset: "designerEmail", targetId: "email-1",
        patch: { templateId: "~template" },
      },
    ], value => value.startsWith("~") ? undefined : value);

    let result = await new MarketoEmailDesignerImpl(ctx).listEmails("1", {
      templateId: "~template",
      ...identityFilters,
    });

    expect(result).toMatchObject({ totalItems, pageIndex: 0, pageSize: 20 });
    expect(result.items.map(item => item.id)).toEqual(["~matching", "email-1"]);
    expect(providerFilters).toBe(0);
  });

  it.each([
    ["isCreatedByMe", { isCreatedByMe: true }],
    ["isCreatedByMe and isModifiedByMe", { isCreatedByMe: true, isModifiedByMe: true }],
  ] as const)("rejects unresolved pending templates with %s before provider access", async (_label, identityFilters) => {
    ...
    ctx.client = async () => {
      providerAccesses++;
      return {} as MarketoClient;
    };

    await expect(new MarketoEmailDesignerImpl(ctx).listEmails("1", {
      templateId: "~template",
      ...identityFilters,
    })).rejects.toThrow(/isCreatedByMe.*templateId.*pending creation/);
  ...
  it("excludes request-only settings from lifecycle and delete snapshot comparisons", () => {
    let provider = { name: "Email", settings: { isOperational: false } };
    let requestOnly = { brandedDomain: "click.example.com", dedicatedIp: "192.0.2.1" };
    let lifecycle = updateDesignerCloneSnapshot(
      designerCloneSnapshot(provider),
      { settings: requestOnly },
    );
    let deletion = updateDesignerDeleteSnapshot(
      designerDeleteSnapshot(provider),
      { settings: requestOnly },
    );
    expect(lifecycle.settings).toEqual({ present: true, value: provider.settings });
    expect(deletion.settings).toEqual({ present: true, value: provider.settings });

    // Previously persisted snapshots are canonicalized again when compared.
    lifecycle.settings = { present: true, value: { ...provider.settings, ...requestOnly } };
    deletion.settings = { present: true, value: { ...provider.settings, ...requestOnly } };

    expect(matchesDesignerCloneSnapshot(provider, lifecycle)).toBe(true);
    expect(matchesDesignerDeleteSnapshot(provider, deletion)).toBe(true);
    expect(designerCloneSnapshot({ settings: requestOnly }).settings).toEqual({ present: false });
    expect(designerDeleteSnapshot({ name: "Email", settings: requestOnly }).settings)
