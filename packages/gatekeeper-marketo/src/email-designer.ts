...
export class MarketoEmailDesignerImpl extends RpcTarget {
  ...
  async #list(kind: EmailDesignerKind, workspaceId: string, options: MarketoDesignerListOptions & { templateId?: string; fragmentType?: string }) {
    ...
    let localTemplateFilter = templateId !== undefined && providerTemplateId === undefined;
    if (localTemplateFilter && options.isCreatedByMe) {
      throw new Error("isCreatedByMe cannot be combined with a templateId that is still pending creation.");
    }
    ...
    if (pending.length === 0) {
      if (localTemplateFilter) {
        await this.#ctx.observe(`List Marketo designer ${kind}`, `Read 0 asset(s) in workspace ${workspace}.`);
        return { items: [], totalItems: 0, pageIndex, pageSize };
      }
    ...
    let exhausted = localTemplateFilter;
    let upstreamPage = 0;
    if (localTemplateFilter) upstreamTotal = 0;
