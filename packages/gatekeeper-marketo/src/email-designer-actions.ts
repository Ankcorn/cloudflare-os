...
/** Whether a persisted clone snapshot contains every comparison field. */
export function validDesignerCloneSnapshot(value: unknown): value is DesignerCloneSnapshot {
  return validDesignerSnapshot(value, SOURCE_COMPARISON_FIELDS);
}
...
function canonicalDesignerRecord(source: Record<string, unknown>): Record<string, unknown> {
  let settings = source.settings;
  let observableSettings = settings && typeof settings === "object" && !Array.isArray(settings)
    ? Object.fromEntries(Object.entries(settings).filter(([key]) => !REQUEST_ONLY_SETTINGS.has(key)))
    : settings;
  return jsonValue({
    ...source,
    ...
    appData: source.appData && typeof source.appData === "object" && !Array.isArray(source.appData)
      ...
      : source.appData,
    settings: observableSettings && typeof observableSettings === "object" &&
        !Array.isArray(observableSettings) && Object.keys(observableSettings).length === 0
      ? undefined
      : observableSettings,
  }) as Record<string, unknown>;
}

function designerSnapshot<Field extends string>(
  source: Record<string, unknown>,
  fields: readonly Field[],
): { [K in Field]: DesignerSnapshotValue } {
  let canonical = canonicalDesignerRecord(source);
  return Object.fromEntries(fields.map(field => [
    field,
    canonical[field] === undefined
      ? { present: false }
      : { present: true, value: canonical[field] },
  ])) as { [K in Field]: DesignerSnapshotValue };
}

function designerSnapshotRecord<Field extends string>(
  snapshot: { [K in Field]: DesignerSnapshotValue },
  fields: readonly Field[],
...
export function designerCloneSnapshotRecord(snapshot: DesignerCloneSnapshot): Record<DesignerCloneField, unknown> {
  return designerSnapshotRecord(snapshot, SOURCE_COMPARISON_FIELDS);
}
...
export function designerCloneSnapshot(source: Record<string, unknown>): DesignerCloneSnapshot {
  return designerSnapshot(source, SOURCE_COMPARISON_FIELDS);
}
...
export function designerDeleteSnapshot(source: Record<string, unknown>): DesignerDeleteSnapshot {
  return designerSnapshot(source, DELETE_COMPARISON_FIELDS);
}
...
export function designerDeleteSnapshotRecord(
  ...
  return designerSnapshotRecord(snapshot, DELETE_COMPARISON_FIELDS);
}
...
export function matchesDesignerDeleteSnapshot(
  ...
  return JSON.stringify(designerDeleteSnapshot(target)) ===
    JSON.stringify(designerDeleteSnapshot(designerDeleteSnapshotRecord(snapshot)));
}
...
export function matchesDesignerCloneSnapshot(
  ...
  return JSON.stringify(designerCloneSnapshot(source)) ===
    JSON.stringify(designerCloneSnapshot(designerCloneSnapshotRecord(snapshot)));
}
...
export function matchesDesignerCloneConfiguration(
  ...
  let canonicalSourceSnapshot = designerCloneSnapshot(designerCloneSnapshotRecord(sourceSnapshot));
  return INHERITED_CLONE_FIELDS.every(field =>
    JSON.stringify(createdSnapshot[field]) === JSON.stringify(canonicalSourceSnapshot[field]));
}
...
async function verifyCreation(
  ...
  if (!created || String(created.id) !== id ||
      !containsApproved(canonicalDesignerRecord(created as Record<string, unknown>), canonicalDesignerRecord(approved)) ||
