...
function workshopConfig(
  ...
  // Most integration tests do not execute Gadget code, so avoid requiring the Worker Loader unless
  // the caller is driving a production agent session that can use executeCode.
  ...
  const server = createTestHarness({
    ...
    workers: [
      { config: workshopConfig(
          gatekeepers, opts.enableGadgetExecution ?? false, opts.patchWorkshop) },
