...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (relPath === "/oauth") {
      ...
      if (error) {
        return new Response("Spotify authorization failed or was denied. Please restart the connection flow from Gadgets.", {
          status: 400,
...
export class GatekeeperVendor extends WorkerEntrypoint<Env> implements GatekeeperVendorIface {
  async describe(): Promise<VendorDescription> {
    return {
      ...
      description:
        "Connect your Spotify account so Gadgets can search the catalog, read and edit your " +
