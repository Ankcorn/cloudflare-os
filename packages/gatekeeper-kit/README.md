...
| `./serial-queue` | FIFO serialization of asynchronous mutations. | A gatekeeper must prevent overlapping updates to one resource. |

## Internal modules

The package does not export `kv`, `positive-int`, `per-storage`, `single-flight`,
