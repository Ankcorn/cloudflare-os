...
/**
 * The catalog the library advertises: every enabled collection, then up to
 * AGENT_SKILL_CATALOG_MAX_ENTRIES skills. Collections lead because both this bound and the
 * Workshop's re-clamp drop from the tail, so a library with hundreds of skills cannot push them out.
 * The merged list is deliberately left unsorted -- the Workshop sorts whatever survives, so sorting
 * here would only decide alphabetically which entries lose.
 */
