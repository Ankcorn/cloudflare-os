...
export class ContextAccount
  ...
  /**
   * Mint the trivial verifier used by the low-stakes observer policy.
   *
...
export class ContextGatekeeper
  ...
  /**
   * Accepts collaborators under the low-stakes observer policy (`docs/observers.md` §9.1 Strategy
   * D). Context collections hold skills and reference documents rather than secrets, and a private
   * collection cannot be shared at all — so per-collection verification could never pass for a
   * collaborator, and only ever blocked sharing a workspace that had read one. Reads are still
