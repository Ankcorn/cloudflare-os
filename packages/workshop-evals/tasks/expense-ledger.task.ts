 ...
 *
 * Almost every check is an invariant rather than a hand-computed constant: balances must sum to
 * zero, transfers must clear them, and split shares must total the expense exactly. Invariants
 * survive a task author's arithmetic slip and still fail loudly on the bug they describe — the
 * classic one here being per-share rounding that quietly loses a cent.
 */
