...
- audit() -> { movementCount: integer, skuCount: integer, reconciles: boolean }
  ...
  reconciles is true only if, for every SKU, stock() equals the sum of that SKU's history().`,
    verify: async verifier => {
      ...
      // Proves the restart mechanism itself before anything relies on it. An abort invalidates every
      // outstanding stub, so a stub that still answers means nothing restarted and every durability
      // check below would be passing vacuously.
      ...
      await verifier.check("survives-repeated-server-restarts", async () => {
        ...
        for (let round = 0; round < 4; round++) {
          ...
          // Every stub taken above is invalidated here, which is why each round reconnects.
          await verifier.restart();
        }
        using api = await verifier.connect<StockApi>("Stock");
        const bolt = await api.stock({ sku: "bolt" });
      ...
      // A duplicate index kept in memory rather than storage passes the earlier check and fails
      // this one, which is the whole point of asking again on the far side of a restart.
