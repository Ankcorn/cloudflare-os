 ...
 *
 * The prompt states the *requirement* (never exceed capacity, people book simultaneously) and not
 * the technique, which is exactly what a user would say — and it matters that it does: GLM 5.2
 * passed by making the check and the insert one synchronous SQL sequence with no await between
 * them, plus a BEFORE INSERT trigger that aborts at capacity. A check asserting "uses a mutation
...
- slotStatus({ slotId: string })
  ...
  personIds lists the people currently holding a place, and its length always equals bookedCount.`,
    verify: async verifier => {
      ...
      // The point of the task. Ten bookings dispatched without awaiting in between arrive as ten
      // interleaved RPC calls, so a check-then-write implementation oversells the slot here.
