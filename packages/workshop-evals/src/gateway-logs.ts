import { z } from "zod";
import type { WorkshopGatewayUsage } from "./task.js";
...
const GatewayEnvironmentSchema = z.object({
  ...
  WORKSHOP_EVAL_GATEWAY_ID: z.string().min(1),
});
...
export function parseGatewayEnvironment(environment: NodeJS.ProcessEnv = process.env): GatewayConfig {
  ...
  return {
    ...
    gatewayId: parsed.data.WORKSHOP_EVAL_GATEWAY_ID,
  };
}
...
export function incompleteGatewayUsage(): WorkshopGatewayUsage {
  return {
    complete: false,
...
/** Exact local filter identifying the outcome judge request. */
export type JudgeLogFilter = {
  /** Stable eval run ID sent by the judge. */
  runId: string;
};
...
function aggregate(logs: readonly GatewayLog[]): WorkshopGatewayUsage {
  return logs.reduce<WorkshopGatewayUsage>((usage, log) => ({
  ...
  }), { ...incompleteGatewayUsage(), complete: logs.length > 0 });
}
...
async function listLogs(
  ...
  for (let page = 1;; page++) {
    const entries = await listLogPage(config, startDate, page, fetchBoundary);
    for (const entry of entries) {
      if (!logs.has(entry.id)) logs.set(entry.id, entry);
    }
    if (entries.length < 100) return [...logs.values()];
  }
}
...
async function pollLogs(
    ...
    sleep: SleepBoundary): Promise<GatewayLog[]> {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (costsAvailable && stableCount >= stableAttempts) return latest;
...
): Promise<WorkshopGatewayUsage> {
  const logs = await pollLogs(config, (log, metadata) =>
...
/** Polls and aggregates the judge request identified by stable eval metadata. */
export async function collectJudgeGatewayUsage(
    config: GatewayConfig,
    filter: JudgeLogFilter,
    options: GatewayPollOptions = {},
    fetchBoundary: FetchBoundary = fetch,
    sleep: SleepBoundary = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds)),
): Promise<WorkshopGatewayUsage> {
  const logs = await pollLogs(config, (_log, metadata) =>
    metadata?.evalRunId === filter.runId && metadata.purpose === "judge",
  options, fetchBoundary, sleep);
  return aggregate(logs);
}

/** Metadata attached to the judge inference request and used for exact log collection. */
export function judgeGatewayMetadata(runId: string): Record<string, string> {
  return { evalRunId: runId, purpose: "judge" };
}
