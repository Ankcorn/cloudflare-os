...
export class SharingManager {
  ...
  *#listLinks(): Generator<ShareLinkRecord> {
   ...
   * redemption too. It runs synchronously with the storage write, after every await, and a throw
   * persists nothing. It is NOT invoked for an already-*confirmed* edge (an existing grant, not
   * a new one) nor for unknown/revoked keys.
   *
   * Returns the redeemed link's id when there is now a pending edge for the caller to settle --
   * whether this call added it, or adopted one a concurrent (or crashed) redemption of the same
   * link left mid-verification -- along with a fresh `attemptId`, this call's own claim on the
   * shared pending edge. The caller passes the attemptId to `revertShareKeyRedemption` so a
   * failure withdraws only its own claim; `confirmShareKeyRedemption` clears every claim.
  ...
  async redeemShareKey(opts: {
    ...
    if (!existing) {
      ...
      // than blindly put, which could overwrite (and, on this open's later failure, revert) an
      // edge the concurrent open already confirmed.
   ...
   * wart: the re-add grants no incremental authority -- the recipient holds the still-live link
   * and can re-redeem it manually whether or not this confirm lands -- and a removal that
   * visibly affects anyone restarts the DO promptly, killing a parked open before it confirms.
   * The durable exclusion is revoking the link: effective-role resolution excludes revoked
   * links, so an edge confirmed after its link was revoked grants nothing (it lingers inert,
   ...
   * redeeming open()'s await windows. A policy change landing in that window must refuse the
   * grant, so the check runs again in this synchronous block, before the pending flag is
   * cleared or the missing edge re-added; a throw persists nothing. NOT invoked for an
   * already-confirmed edge (an existing grant, not a new one -- matching redeemShareKey).
   */
   ...
   * or adopted, severing the edge only once no claims remain: a failed attempt must not yank the
   * edge out from under a sibling still mid-verification. Used when the redeeming user is
   * refused *after* redemption, so a refused recipient never persists in the sharing graph; they
   * can redeem the same key again once whatever refused them is fixed. An edge a concurrent open
