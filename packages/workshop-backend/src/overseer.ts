...
class OverseerImpl implements AgentHooks {
  ...
  async authorizeCollaborator(
    ...
    // inside the per-profile verification lock. For a redemption the role counts the pending
    // edge, exactly as the entry check did -- the edge is confirmed only below, on success.
    let commitGate = () => {
      if (!sharing.getEffectiveRole(profileId, opts.pendingLinkId)) {
        throw new Error(
    ...
    // landing in the await gaps *after* the gate ran -- the role collapses to null and the
    // caller rejects. For a redemption in that narrow window the just-confirmed edge lingers
