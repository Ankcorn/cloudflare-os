...
class OverseerImpl implements AgentHooks {
  ...
  // The authorization gate every non-owner entry point must pass through: resolve the caller's
  // effective role, then verify them as an observer of everything this workspace has read.
  // Returns null for no access; verification failures throw. A caller that requires at least
  // `requireRole` (e.g. receiveExternalMessage needs "build") passes it so an insufficient role
  // is denied *before* verification runs -- otherwise the caller would be verified (real
  // addObserver calls, a persisted observer record) only to be turned away, or worse, told to fix
  // a verification failure that can never grant them access. `configureCb` is forwarded to
  ...
  async #ensureObserverLocked(
    ...
    // that window so #decideExcludeObservers can fail closed on it. The commit gate, the put,
