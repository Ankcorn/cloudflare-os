...
export function makeOverseerStorage(storage: DurableObjectStorage) {
  return createTypedStorage(storage, {
    singletons: {
      ...
      // requiring manual approval (never auto-approved; see autoApprovalRule). The key on disk
      // predates the flag's rename.
...
class OverseerImpl implements AgentHooks {
  ...
  // pass restarts every live session (see #restartIfShared) -- but #inScopeGatekeepers skips
  ...
  #assertUnverifiableProducerUnshared(gatekeeper: GatekeeperRecord, sharing: SharingManager): void {
    ...
    throw new Error(
        ...
        "workspace is shared. Its collaborators must be removed before this data can be read.");
  }

  // The connection ids this workspace has read restricted data through: the producers the latch
  // guards. Derived by scanning the action log for observations whose description carries
  // `containsRestrictedData`, since nothing else records which connection a latched read came
  // through. The callers are cold paths (connection removal, sharing mutators) plus submitAction
  // and applyPendingAction -- but both scan only while latched (the unlatched path short-circuits
  // before the scan), latched manual approvals are human-bounded, and the auto-approval drainer
  ...
  // let a never-verified party see the data -- the record is what observer verification runs
  // against at every open, and for an unverifiable record it is what refuses the producer's reads
  ...
  removalBlockedByRestrictedData(id: WorkpieceId, sharing: SharingManager): boolean {
    if (!this.storage.containsRestrictedData.get()) return false;
  ...
  assertNewSharingAllowed(): void {
    if (!this.storage.containsRestrictedData.get()) return;
  ...
  async #enforceExcludeObservers(observerIds: string[]): Promise<void> {
    ...
    for (let observerId of observerIds) {
      ...
      //   removeObserver fan-out, so a re-granted profile's *replacement* observer record can be
      //   deleted here, after which exclusions naming the new id silently no-op fail-open.
  ...
  //
  // TODO(known-risk): scoping by role means a "use" collaborator is never verified against a
  //   producer that no gadget binds -- yet restricted data read from such a producer can reach
  //   gadget state and the UI they drive, because provenance is not tracked past the observation.
  //   Deliberately accepted for v1; the required fix (verify every restricted producer, or isolate
  //   restricted data by provenance) is recorded under "Known security risk -- never-bound
  //   producers" in plans/restricted-data-sharing.md, and worked through in docs/observers.md
  //   edge case 4.
  ...
  async ensureObserver(
    ...
    try {
      ...
      //
      // Scheduled first, ahead of the equally best-effort rollback below: it is fire-and-forget, so
      // ordering it first takes a gatekeeper RPC fan-out off the path between determining the denial
      // and the abort, and the rollback then proceeds under the scheduled restart.
      //
      // TODO(known-limitation): a re-prompt the client never answers still defers this
      //   indefinitely, since `configureCb.configure()` is awaited above and the failure is only
      //   terminal once the re-prompt budget is spent. Stalling the modal is equivalent to never
      //   re-opening, which docs/observers.md edge case 3 already accepts, so it grants no access
      //   the collaborator doesn't already hold. See plans/restricted-data-sharing.md, Known
      //   limitations.
      if (scrubbedCoverage) {
