...
import { createTypedStorage, collection, keyString, type Collection } from "@gadgets/typed-storage";
...
// restart are injected, keeping this testable over a mock storage (same pattern as
// auto-approval.ts). Unlike the drainer this has no cross-call state, so it is a plain function.

const rejectionLogger = createWorkshopLogger("workshop.action.rejection");
 ...
 *
 * Returns the gatekeeper's result so the caller can gate follow-up work on it -- in particular,
 * a requested restart signals that remaining pending actions may have been staged on top of
 * now-invalid simulated state, so the caller should NOT immediately auto-apply them.
 *
 * Concurrency notes:
...
export async function rejectPendingAction(
    ...
    restartGadget?: RestartGadgetFn): Promise<GatekeeperRejectResult> {
  ...
  // Re-gets the record and marks it rejected iff it is still pending, returning false if it
  // raced away. The re-get is load-bearing for the primary (`record` was read before the
  // gatekeeper await and may be stale; the gatekeeper side has already cleaned up
  // unconditionally) and keeps the cascade mutation uniform.
  let markRejected = (id: number, cascadedFrom?: number): boolean => {
    let fresh = storage.actions.get(id);
    if (!fresh || fresh.type !== "action" || fresh.state !== "pending") return false;
    fresh.state = "rejected";
    fresh.appliedAt = new Date();  // existing convention: doubles as "resolvedAt" for rejections
    fresh.resolvedBy = resolvedBy;
    if (cascadedFrom !== undefined) fresh.cascadedFrom = cascadedFrom;
    storage.actions.put(fresh);
    return true;
  };

  if (!markRejected(record.id)) {
    rejectionLogger.warn("rejected action was no longer pending when marking it", {
  ...
  if (rejectKeys.length > 0) {
    ...
    for (let candidate of cascade) {
      markRejected(candidate.id, record.id);
    }

    // The primary's own key is already handled, so it doesn't count as "unmatched".
    let matched = new Set(cascade.map((rec) => rec.action));
    let unmatched = rejectKeys.filter((key) => !matched.has(key) && key !== record.action);
    if (unmatched.length > 0) {
      rejectionLogger.warn(
          `gatekeeper cascade-rejected actions with no matching pending record: ${unmatched.join(", ")}`,
  ...
  if (result && result.restart) {
    if (restartGadget) {
      try {
        ...
        rejectionLogger.warn("failed to restart gadget after rejection", {
      ...
      rejectionLogger.info("gatekeeper requested restart but the action has no restartable gadget", {
...
class OverseerImpl implements AgentHooks {
  ...
  async applyPendingAction(record: ActionRecord & {type: "action"},
    ...
    // The apply succeeded, so "approved" is the truthful record state and the write below stands
    // regardless of what raced in during the await: world wins. A conforming gatekeeper can't
    // cascade-reject an action it just applied, so a raced record here fingers a nonconforming
    // gatekeeper -- detect and log it rather than silently overwriting.
    let fresh = this.storage.actions.get(record.id);
    if (!fresh || fresh.type !== "action" || fresh.state !== "pending") {
      logger.error("action raced to a non-pending state while its apply was in flight", {
        event: "action.apply.raced", actionId: record.id, gatekeeperId: record.gatekeeperId,
      });
    }
...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async rejectAction(id: number): Promise<void> {
    ...
    let result = await rejectPendingAction(
    ...
    // suffices: the cascade is same-gatekeeper by construction). Skipping the eager drain after a
    // restart is only a timing choice, not a correctness gate: an already-in-flight drain may
    // still finish its pass, and restart-surviving auto-eligible actions will auto-apply at the
    // next trigger (submit/approve/rule-enable, or the restarted gadget re-submitting) anyway.
    // Never applying an invalidated action is enforced elsewhere -- the gatekeeper's cascade
    // deletes it on its side (a later apply throws "Unknown action" and stops the drainer), and
    // the drainer re-gets each record before applying.
    if (!result?.restart) {
      this.impl.ctx.waitUntil(this.impl.drainAutoApprovals(action.gatekeeperId));
    }
  }
