...
describe("receiveExternalMessage's commit-time authorization re-check", () => {
  ...
  it("denies the existing-chat path without committing the message", async () => {
    ...
    await runInDurableObject(stub, async (instance: OverseerDurableObject) => {
      ...
      // this case would pass vacuously: materialization would no-op, and a materialization
      // running *before* the authorization re-check -- durably writing a "changes" message,
      // retiring the row, and setting hasProposedChanges on a denied submission -- would go
      // undetected.
