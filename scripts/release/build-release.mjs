...
// would upload it (dry-run + outdir, with the repo's pinned wrangler), plus two workshop-frontend
// asset builds (password-auth and Access modes), plus the release manifest that describes it all.
//
...
// Builds the frontend once per auth-mode variant. VITE_CF_ACCESS_MODE is a build-time flag
// (workshop-frontend/src/useAuth.ts), so the release carries both builds and the deploy
// workflow picks per instance.
function buildFrontendVariant(variant) {
  const env = { ...process.env };
  delete env.VITE_CF_ACCESS_MODE;
  if (variant === "access") env.VITE_CF_ACCESS_MODE = "true";
...
function main() {
  ...
  // 1. Frontend variants first: the router's wrangler.jsonc points its assets directory at
  //    workshop-frontend/dist, so it must exist before the router's dry-run.
  const assetVariants = {
    password: buildFrontendVariant("password"),
    access: buildFrontendVariant("access"),
  };
