...
export function buildWorkerEntry({ pkgName, config, mainModule, modules, deployInputs }) {
  ...
  if (config.assets) {
    ...
    assetsConfig = {
      ...
      // Two frontend builds per release until VITE_CF_ACCESS_MODE becomes a runtime flag; the
      // deploy workflow picks the variant matching the instance's auth mode. Filled by the
      // caller (build-release) with { "<variant>": { manifest: { "/path": { hash, size } } } }.
      variants: {},
    };
  }
