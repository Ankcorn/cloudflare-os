...
test("collectAssets builds an upload-session-shaped manifest with deduped blobs", () => {
  const password = collectAssets(join(TESTDATA, "fixture-assets", "password"));
  const access = collectAssets(join(TESTDATA, "fixture-assets", "access"));

  assert.deepEqual(Object.keys(password.manifest).toSorted(),
      ["/assets/app.js", "/assets/shared.css", "/index.html"]);
  for (const entry of Object.values(password.manifest)) {
    assert.equal(entry.hash.length, 32);
    assert.ok(entry.size > 0);
  }
  // shared.css is byte-identical across variants -> same content key; app.js differs.
  assert.equal(password.manifest["/assets/shared.css"].hash,
      access.manifest["/assets/shared.css"].hash);
  assert.notEqual(password.manifest["/assets/app.js"].hash,
      access.manifest["/assets/app.js"].hash);
  assert.ok(password.blobs.has(password.manifest["/index.html"].hash));
});
