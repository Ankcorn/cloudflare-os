...
test("over-long preview names truncate to a stable hashed form", () => {
  ...
  assert.match(slug, /^renovate-[0-9a-f]{8}$/);
...
test("PREVIEW_NAME is what CI passes, and it wins over the local git fallback", () => {
  assert.equal(resolvePreviewName({ name: "pr-42" }), "pr-42");
  assert.equal(resolvePreviewName({ name: "PR/42" }), "pr-42");
});
...
test("every deployable package gets a preview config, on the configured account", () => {
  ...
  for (const [name, config] of configs) {
    ...
    assert.ok(config.previews, `${name}: no previews block`);
    // The preview account is shared with a deployment that uses these packages' bare names, so
    // an unprefixed name here is a worker that already belongs to someone else.
    assert.equal(config.name, previewWorkerName(name), `${name}: unprefixed worker name`);
  }
});
...
test("every gatekeeper is bound to the backend by RPC and to the router by HTTP", () => {
  ...
  // A service binding names a worker, so these carry the prefix while the binding name — which
  // is what the router and the backend actually scan for — does not.
  ...
  assert.deepEqual(backend.map((service) => service.service),
      gatekeepers.map(previewWorkerName));
  ...
  assert.deepEqual(router.map((service) => service.service),
      ["workshop-backend", ...gatekeepers].map(previewWorkerName),
...
test("each preview's context collections are namespaced to that preview", () => {
  ...
  const context = previewsOf(configs, "workshop-backend").services
      ?.find((service) => service.service === previewWorkerName("gatekeeper-context"));
