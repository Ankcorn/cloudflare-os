...
interface ListedPreview {
  /** The preview name, which is the slugified branch it was deployed from. */
  name: string;
  created_on?: string;
  created_at?: string;
  modified_on?: string;
...
/** One pull request as the sweep reads it: the branch it is for, and whether it is still open. */
interface ListedPullRequest {
  /** "open", or "closed" — which covers merged. */
  state: string;
  /** When it last changed, used to stop paginating. */
  updated_at?: string;
  /** Its head branch, whose slug is the preview name. */
  head?: { ref?: string };
}
...
function preparePreviewWrangler(): PreviewWrangler {
  ...
  const installDir = mkdtempSync(join(tmpdir(), `${PREVIEW_WORKER_PREFIX}preview-wrangler-`));
  try {
...
function writePreviewComment(
  ...
  const dashboardUrl = `https://dash.cloudflare.com/${accountId}/workers/services/view/` +
      `${previewWorkerName("router")}/production/previews/${slug}`;
  // The slug is the branch, so it is worth showing: it is the first label of the URL below.
  const comment = [
    `### Preview: \`${slug}\``,
    "",
    baseUrl,
...
async function deploy({ dryRun }: { dryRun: boolean }): Promise<void> {
  ...
  if (dryRun) {
    ...
    for (const pkg of gatekeepers) {
      console.log(`    ${previewWorkerName(pkg.name)} ` +
          `(no hostname; served at ${baseUrl}/gatekeeper/${gatekeeperShortName(pkg.name)})`);
    }
    console.log(`  tier 2: ${previewWorkerName(backend.name)} (no hostname; served at ` +
        `${baseUrl}/api), bound to the tier 1 previews, holding the ` +
        `${Object.keys(secrets).join(", ")} secrets`);
    console.log(`  tier 3: ${previewWorkerName(router.name)} -> ${baseUrl}, ` +
  ...
  try {
    ...
    const gatekeeperPreviews = await mapWithConcurrency(gatekeepers, GATEKEEPER_CONCURRENCY,
        async (pkg) => {
          ...
          return [previewWorkerName(pkg.name), preview.id];
        });
    ...
    patchPreviewServiceBindings(router, {
      [previewWorkerName(backend.name)]: backendPreview.id,
    });
...
async function remove({ dryRun }: { dryRun: boolean }): Promise<void> {
  ...
  try {
    ...
    // Dependents first, so nothing is left bound to a preview that no longer exists.
    await deletePreview(router, previewName, wrangler.command);
    await deletePreview(backend, previewName, wrangler.command);
    await mapWithConcurrency(gatekeepers, GATEKEEPER_CONCURRENCY,
        (pkg) => deletePreview(pkg, previewName, wrangler.command));
  } finally {
    wrangler.cleanup();
  }
}
...
// Enough pages to cover every pull request touched inside PREVIEW_MAX_AGE_DAYS, with the loop
// stopping early on the first page that predates it.
const PULL_REQUEST_PAGES = 10;
...
async function githubApi(repo: string, path: string): Promise<unknown> {
  const headers: Record<string, string> = {
    Accept: "application/vnd.github+json",
    "User-Agent": `${PREVIEW_WORKER_PREFIX}preview-sweep`,
  };
  if (process.env.GITHUB_TOKEN) headers.Authorization = `Bearer ${process.env.GITHUB_TOKEN}`;

  const response = await fetch(`https://api.github.com/repos/${repo}${path}`, { headers });
  if (!response.ok) {
    throw new Error(`GitHub API GET /repos/${repo}${path} failed with ${response.status}`);
  }
  return response.json();
}

/**
 * Every recent pull request's state, keyed by the preview name its branch produces.
 *
 * A preview is named for its branch, not its PR number, so this is what maps one back to the other:
 * the same slugify the deploy applies to `github.head_ref`. Newest-first and bounded, because a
 * preview old enough to have fallen off the end is swept on age regardless — and where a branch has
 * had more than one PR, the most recent one is the one that decides. "closed" covers merged.
 */
async function pullRequestStatesByPreview(repo: string): Promise<Map<string, string>> {
  const states = new Map<string, string>();
  for (let page = 1; page <= PULL_REQUEST_PAGES; page++) {
    let pulls: ListedPullRequest[];
    try {
      const body = await githubApi(repo,
          `/pulls?state=all&sort=updated&direction=desc&per_page=${PULL_REQUEST_PAGE_SIZE}` +
          `&page=${page}`);
      if (!Array.isArray(body)) {
        throw new Error(`expected a list of pull requests, got ${typeof body}`);
      }
      pulls = body as ListedPullRequest[];
    } catch (error) {
      // Degrade rather than abort. This job exists to stop an abandoned preview from leaking a KV
      // pair and an R2 bucket, and it deletes nothing until this whole map is built — so a
      // transient 403 (secondary rate limit) or 5xx on one page would otherwise defeat the sweep
      // until tomorrow night. Pages that did arrive still count, and every preview this one would
      // have matched falls back to being judged on age alone, which never deletes a preview that
      // an open pull request is still using.
      console.warn(`GitHub page ${page} of ${repo}'s pull requests failed ` +
          `(${describe(error)}); the previews it would have matched are swept on age alone.`);
      break;
    }
    for (const pull of pulls) {
      const ref = pull.head?.ref;
      if (!ref) continue;
      const name = slugifyPreviewName(ref);
      if (!states.has(name)) states.set(name, pull.state);
    }
    if (pulls.length < PULL_REQUEST_PAGE_SIZE) break;
    const oldest = pulls.at(-1)?.updated_at;
    if (oldest && daysSince(oldest) > PREVIEW_MAX_AGE_DAYS) break;
  }
  return states;
}
...
function staleReasons(
  preview: ListedPreview,
  states: Map<string, string> | undefined,
): string[] {
  const reasons: string[] = [];
  const age = ageInDays(preview);
  if (age !== null && age > PREVIEW_MAX_AGE_DAYS) {
    reasons.push(`${age.toFixed(1)} days old`);
  }

  // A preview whose name matches no recent pull request — one pushed by hand from a local branch,
  // or left behind when a branch was renamed mid-review — is swept on age alone.
  const state = states?.get(preview.name);
  if (state && state !== "open") reasons.push(`its pull request is ${state}`);
  return reasons;
}
...
async function sweep({ dryRun }: { dryRun: boolean }): Promise<void> {
  ...
  const states = repo ? await pullRequestStatesByPreview(repo) : undefined;
  ...
  if (!Array.isArray(previews)) {
    throw new Error(`Expected a list of previews, got ${JSON.stringify(previews)?.slice(0, 200)}`);
  }
  console.log(`\n${previews.length} preview(s) on ${previewWorkerName(router.name)}.`);

  const stale: { name: string; reasons: string[] }[] = [];
  for (const preview of previews as ListedPreview[]) {
    const reasons = staleReasons(preview, states);
    if (reasons.length > 0) stale.push({ name: preview.name, reasons });
    else console.log(`  keeping ${preview.name}`);
  }
  ...
  for (const { name, reasons } of stale) {
  ...
  try {
    ...
    for (const { name } of stale) {
      await deletePreview(router, name, wrangler.command);
      await deletePreview(backend, name, wrangler.command);
      await mapWithConcurrency(gatekeepers, GATEKEEPER_CONCURRENCY,
          (pkg) => deletePreview(pkg, name, wrangler.command));
    }
  } finally {
    wrangler.cleanup();
  }
  console.log(`\nSwept ${stale.length} preview(s).`);
}
