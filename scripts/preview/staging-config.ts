 ...
 * `<worker>-<preview>-<binding lowercased>`, so the longest of those decides it:
 * 63 - len("os-public-workshop-backend") - len("blueprint-content") - 2 separators.
 */
export const MAX_PREVIEW_NAME_LENGTH = 18;
const PREVIEW_NAME_HASH_LENGTH = 8;

const GATEKEEPER_PREFIX = "gatekeeper-";

/**
 * Namespace for every worker, resource and scratch path this script creates. The preview account
 * is shared with a separate deployment that deploys these same packages under their bare names —
 * so without a prefix, most of the eighteen baselines would be workers that deployment already
 * owns, and PR previews would be branches of them. Renaming after the fact orphans the old worker
 * rather than moving it, so this is worth getting right before the first deploy.
 */
export const PREVIEW_WORKER_PREFIX = "os-public-";
...
/**
 * The worker a package is deployed as: `router` -> `os-public-router`. Service bindings name
 * workers, not packages, so everything that wires one worker to another goes through this.
 */
export function previewWorkerName(pkgName: string): string {
  return `${PREVIEW_WORKER_PREFIX}${pkgName}`;
}
...
export function previewUrlFor(
  ...
  return `https://${previewName}-${previewWorkerName(pkgName)}.${workersDevHost}`;
}
...
export function slugifyPreviewName(raw: string): string {
  const slug = raw.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  if (!slug) return "preview";
  if (slug.length <= MAX_PREVIEW_NAME_LENGTH) return slug;

  const hash = createHash("sha256").update(raw).digest("hex").slice(0, PREVIEW_NAME_HASH_LENGTH);
  const prefix = slug.slice(0, MAX_PREVIEW_NAME_LENGTH - PREVIEW_NAME_HASH_LENGTH - 1)
      .replace(/-+$/g, "");
  return `${prefix}-${hash}`;
}
...
/**
 * The preview name, which is also the first label of its hostname — so CI passes the pull
 * request's head branch and a preview reads as `my-branch-os-public-router.<subdomain>`, rather
 * than as a PR number nobody recognizes.
 *
 * The cost is that a preview can no longer be found from a PR number alone: the close-event job
 * passes the same branch (GitHub gives it the head ref on that event too), and the nightly sweep
 * slugifies every recent pull request's head ref to match previews back up — see preview.ts.
 */
export function resolvePreviewName(
  { name = process.env.PREVIEW_NAME }: { name?: string } = {},
): string {
  return slugifyPreviewName(name || localRefName());
}
 ...
 * baseline to another deployment's live bucket on an account we share with it, and would leave
 * the KV bindings with no id at all.
 *
...
function backendGatekeeperServices(gatekeepers: string[], baseUrl: string): PreviewService[] {
  return gatekeepers.map((pkgName) => ({
    ...
    service: previewWorkerName(pkgName),
...
function applyRouter(config: StagingConfig, { gatekeepers }: PreviewContext): void {
  ...
  config.services = [
    { binding: "WORKSHOP_BACKEND", service: previewWorkerName("workshop-backend") },
...
export function buildPreviewConfigs({
  ...
  for (const pkg of packages) {
    ...
    Object.assign(config, {
      name: previewWorkerName(pkg.name),
