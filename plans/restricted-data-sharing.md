...
- A failed re-verification's sever can be deferred indefinitely by the collaborator being
  denied: the scrub is synchronous with the failure, but the restart is scheduled only
  once the failure is terminal, and the re-prompt before that awaits a client-controlled
  `configure()`. Stalling the modal is equivalent to never re-opening, which the lazy
  model already accepts (`docs/observers.md` edge case 3), so it grants no access they do
  not already hold — but it does mean the ~100ms bound does not apply to this trigger. An
  eager restart from inside the scrub was considered and rejected: it kills the in-session
  repair path and needs a latch so N failed producers don't stack N aborts. Marked
  `TODO(known-limitation)` in `ensureObserver`'s catch in `overseer.ts`.
